...
function SignUp({ useReducer, useRef, axios }) {
    ...
    return (
        <div className="container__form container--signin">
            <form onSubmit={(e) => submitSignup(e)} className="form" id="form1">
                <h2 className="form__title">Đăng ký</h2>
                <input type="tel" placeholder="Số điện thoại" value={state.phone} onChange={(e) => setState({ phone: e.target.value })} className="input" required />
                {state.phone !== "" && !/((09|03|07|08|05)+([0-9]{8})\b)/g.test(state.phone) ? (
                    <p className="warningText">Số điện thoại không hợp lệ!</p>
                ) : null}
                <div className="inputPassword">
                    <input type={state.seePassword ? "text" : "password"} placeholder="Mật khẩu" value={state.password} onChange={(e) => setState({ password: e.target.value })} required />
                ...
                <div className="inputPassword">
                    <input type={state.seeRepeatPassword ? "text" : "password"} placeholder="Nhập lại mật khẩu" value={state.repeatPassword} onChange={(e) => setState({ repeatPassword: e.target.value })} required />
                ...
                {state.repeatPassword !== "" && state.repeatPassword !== state.password ? (
                    <p className="warningText">Mật khẩu không trùng khớp!</p>
                ) : null}
