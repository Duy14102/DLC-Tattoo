...
function AddType2Booking({ state, setState, getBooking, toast, toastNow, ToastUpdate, axios, useRef, useEffect, ReactPaginate }) {
    ...
    function newSamplesToBooking(e) {
        ...
        axios(configuration).then((res) => {
            getBooking()
            setState({ modalOpen2: false, addImage: null, addTitle: "", addPrice: "", sessionType2Add: [], modalData3: null, checkTitle: false, warningOverPrice: false, wantType: null, samplesForAdd: null, pageCount2: 6, search2: "", contentSearch2: null, samplesId: [] })
    ...
    return (
        <Modal open={state.modalOpen2} onClose={() => setState({ modalOpen2: false, addImage: null, addTitle: "", addPrice: "", sessionType2Add: [], modalData3: null, checkTitle: false, warningOverPrice: false, wantType: null, samplesForAdd: null, pageCount2: 6, search2: "", contentSearch2: null, samplesId: [] })} center>
            <h4 className='modalTitle'>Thêm hình mẫu</h4>
            <div className='fatherAddType2'>
                {state.wantType ? null : (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-evenly" }}>
                        <button onClick={() => setState({ wantType: 1 })} className='chooseAddTypButtton'>Có sẵn</button>
                        <button onClick={() => setState({ wantType: 2 })} className='chooseAddTypButtton'>Tự thêm</button>
                    </div>
                )}
                {state.wantType === 1 ? (
                    <LoadSamples axios={axios} useRef={useRef} state={state} setState={setState} useEffect={useEffect} ReactPaginate={ReactPaginate} toast={toast} toastNow={toastNow} ToastUpdate={ToastUpdate} getBooking={getBooking} level={1} />
                ) : state.wantType === 2 ? (
                    <form onSubmit={(e) => newSamplesToBooking(e)}>
                        <div className='addType2'>
                            ...
                            <div className='addType2SecondChild'>
                                {state.addImage ? (
                                    <>
                                        <LazyLoadImage alt='Image' src={state.addImage} />
